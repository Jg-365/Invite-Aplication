...
function BirthdayInvitation() {
  ...
  const handleRsvp = (response) => {
    ...
    if (response === "sim") {
      ...
      setStep(2); // Passo 2: Exibe o botão "Não vou"
    }
  };
  ...
  const handleNaoVou = async () => {
    try {
      const inviteId = window.location.pathname.split("/").pop();
      await axios.post(
        `https://pagode-8a84169bad42.herokuapp.com/invite/${inviteId}/rsvp`,
        { rsvp: "não", drink: [] } // Enviar "não" e limpar as bebidas
      );
      setStep(4); // Passo 4: Exibe a tela de despedida após enviar os dados
    } catch (error) {
      console.error("Erro ao enviar dados de 'não vou':", error);
    }
  };
  ...
  return (
    <div className="pageContainer">
      <div className="container">
        ...
        {/* Exibir botão "Não vou" no passo 2 */}
        {step === 2 && rsvp === "não" && (
          <div className="response">
            <h3>Tem certeza que não vai? 😪</h3>
            <button className="submitButton" onClick={handleNaoVou}>
              Não vou
            </button>
          </div>
        )}
